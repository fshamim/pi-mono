...
- Exported `CustomProviderDialog` from `@earendil-works/pi-web-ui` ([#2267](https://github.com/badlogic/pi-mono/issues/2267))
...
- Build `@earendil-works/pi-web-ui` with `tsc` instead of `tsgo` so Lit decorator-based state updates rerender correctly.
...
- **Agent class moved to `@earendil-works/pi-agent-core`**: The `Agent` class, `AgentState`, and related types are no longer exported from this package. Import them from `@earendil-works/pi-agent-core` instead.
...
- **`AppMessage` renamed to `AgentMessage`**: Now imported from `@earendil-works/pi-agent-core`. Custom message types use declaration merging on `CustomAgentMessages` interface.
...
- **`CustomMessages` interface removed**: Use declaration merging on `CustomAgentMessages` from `@earendil-works/pi-agent-core` instead.

- **`agent.appendMessage()` removed**: Use `agent.queueMessage()` instead.

- **Agent event types changed**: `AgentInterface` now handles new event types from `@earendil-works/pi-agent-core`: `message_start`, `message_end`, `message_update`, `turn_start`, `turn_end`, `agent_start`, `agent_end`.
...
- `Agent` class (moved to `@earendil-works/pi-agent-core`)
...
**Before (0.30.x):**
```typescript
import { Agent, ProviderTransport, type AppMessage } from '@earendil-works/pi-web-ui';
