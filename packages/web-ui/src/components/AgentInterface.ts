...
import type { ToolPartialResult } from "../tools/types.ts";
...
export class AgentInterface extends LitElement {
	...
	private findAssistantMessageForToolCall(toolCallId: string): AgentMessage | undefined {
		return this.session?.state.messages.find(
			(message) =>
	...
	private showStreamingToolCall(toolCallId: string): void {
		const message = this.findAssistantMessageForToolCall(toolCallId);
		if (!message || !this._streamingContainer) return;
		this._streamingContainer.isStreaming = this.session?.state.isStreaming || false;
		this._streamingContainer.setMessage(message, true);
	}
		...
		this._unsubscribeSession = this.session.subscribe(async (ev: AgentEvent) => {
			switch (ev.type) {
				...
				case "tool_execution_start":
					this.showStreamingToolCall(ev.toolCallId);
					this.requestUpdate();
					break;
				case "tool_execution_update":
					this.showStreamingToolCall(ev.toolCallId);
					this._partialToolResultsById = new Map(this._partialToolResultsById).set(
						ev.toolCallId,
						ev.partialResult as ToolPartialResult<any>,
					);
					this.requestUpdate();
					break;
				case "message_end": {
					if (ev.message.role === "toolResult" && !ev.message.isError) {
						const nextPartialToolResults = new Map(this._partialToolResultsById);
						nextPartialToolResults.delete(ev.message.toolCallId);
						this._partialToolResultsById = nextPartialToolResults;
					}
				...
				case "agent_end":
					this._partialToolResultsById = new Map();
